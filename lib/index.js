...
async function parse (context, filepath, current, level, isEntry, input_modules) { 
    ...
    if (filepath) {
        ...
        if (file.invalidate) {
            ...
            let { dependencies, transpiled, dynamicDependencies, exports, imports } = ImportExportResolver(code, context);
        ...
        ${context.output.format === 'esm'? 'var __nollup_entry_exports = ' : ''}(function (modules) {
            ...
            let resolve_module = function (parent, number) {
                ...
                module.dependencies.forEach((dep) => {
                    if (!instances[dep].resolved && instances[dep].resolving) {
                        instances[dep].dependencies.forEach(dep => {
                            if (!instances[dep].resolved && !instances[dep].resolving) {
                                resolve_module(module, dep);
                            }
                        });
                        instances[dep].__impl(localRequire, module);
                        instances[dep].resolved = true;
                    }
