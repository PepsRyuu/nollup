...
const isEsm = format => format === 'es' || format === 'esm'
...
async function generate (context, input, dynamicFileName) {
    ...
    if (dynamicFileName) {
        ...
        return [
                ...
                `
        ${(context.files[input.file].exports.length > 0 && isEsm(context.output.format))? 'var __nollup_entry_exports = ' : ''}(function (modules, __nollup__global__) {
            let instances = {};
            let chunks = {};
        ...
        ${isEsm(context.output.format)? context.files[input.file].exports.map(declaration => {
