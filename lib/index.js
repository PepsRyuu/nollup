...
(function (modules) {
    ...
    let require = function (parent, number) {
        if (!instances[number] || instances[number].invalidate) {
            ...
            ${nollup.__bundleHooks.__hooks['module-init'].map(fn => {
                return `
                    (${fn.toString()})(instance)
                `;
            })}

            modules[number](function (dep) {
                return require(instance, dep);
    ...
    ${nollup.__bundleHooks.__hooks['init'].map(fn => {
        return `
            (${fn.toString()})(modules, instances)
        `;
    })}

    return require(null, ${context.files[context.options.input].module_id});
})({
        `,
        files,
        `
...
async function nollup (options) {
    ...
    return {
        invalidate: file => {
            file = resolvePath(file, process.cwd());
...
// Private API for dev middleware.
nollup.__bundleHooks = {
    __hooks: {
        'init': [],
        'module-init': []
    },
