        ...
        try {
            ...
            for (let i = 0; i < bundles.length; i++) {
                ...
                update.output.forEach(obj => {
                    ...
                    let fileSource = obj.isAsset? obj.source : obj.code;
                    ...
                    if (options.virtualWrite) {
                        let vw_root = path.join(options.virtualWrite)
                            .split(path.sep)
                            .filter(s => s !== '')
                            .join('/');

                        let fullFileName = path.join(configs[i].output.dir, fileName)
                            .split(path.sep)
                            .filter(s => s !== '')
                            .join('/');

                        if (fullFileName.startsWith(vw_root)) {
                            fileName = fullFileName.replace(vw_root + '/', '');
                        } else {
                            return;
                        }
                    } 
