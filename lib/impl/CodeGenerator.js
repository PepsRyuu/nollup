...
function getSyntheticExports (context, filePath) {
    ...
    return `if (module.exports.${synthetic}) {
        Object.keys(module.exports.${synthetic}).forEach(prop => {
            prop !== 'default' && __e__(prop, function () { return module.exports.${synthetic}[prop] });
        });
    }`
}

function getLiveBindingGetter (context) {
    return context.liveBindings? `
    __nollup__global__.__nollup__lb__define__ = function (imports, key, getter) {
        !imports.hasOwnProperty(key) && Object.defineProperty(imports, key, {
            get: getter
        });
    }
    ` : '';
...
function generateFile (context, filePath) {
    ...
    return `
        function (__c__, __r__, __d__, __e__) {
            ${context.liveBindings? 
                `var __i__ = {}; ${imports.map(i => `var ${i.importee};`).join('; ')}` 
                : 
                imports.map(i => `var ${i.importee}; ${i.specifiers.map(s => 'var ' + s.local).join(';')};`).join('; ')
            }
            ...
            __d__(function () {
                ${imports.map(i => {
                    return i.specifiers.map(s => {
                        let output;
                        let getter = `${i.importee}()${s.imported === '*'? '' : `.${s.imported}`}`;

                        if (!context.liveBindings) {
                            output = `${s.local} = ${getter}`;
                        } else {
                            output = `__nollup__lb__define__(__i__, "${s.local}", function () { return ${getter} });\n`
                        }

                        if (s.exportFrom) {
                            if (s.imported === '*') {
                                let alias = context.liveBindings? `__i__["ex${i.importee}"]` : `ex${i.importee}`; 
                                output += `;for(var __k__ in ${alias}){__k__ !== "default" && (__e__(__k__, function () { return ${alias}[__k__] }))}`
                            } else {
                                let alias = context.liveBindings? `__i__["${s.local}"]` : s.local;
                                output += `;(__e__("${s.local.slice(3)}", function () { return ${alias} }))`;
                            }
                        }
...
async function generateChunk (context, chunk, bundle, dynamicImports) {
    ...
    if (chunk.isDynamicEntry) {
        return `
            (function (__nollup__global__) {
                var __nollup__import__meta__ = {
                    ${resolveMetaProperties(context, chunk, bundle)}
                };

                ${getLiveBindingGetter(context)}

                __nollup__global__.__nollup_dynamic_require_callback("${chunk.fileName}", ${chunk.__entryModuleId}, {${files}});
        ...
        `   var __nollup__import__meta__ = {
            ...
            var create_module = function (number) {
                ...
                modules[number](function (dep) {
                    ...
                    ${context.liveBindings? `
                    if (!module.exports.hasOwnProperty(bindingName)) {
                        Object.defineProperty(module.exports, bindingName, {
                            configurable: true,
                            enumerable: true,
                            get: bindingValue
                        });
