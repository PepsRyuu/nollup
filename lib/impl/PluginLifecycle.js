...
function handleMetaProperty (context, filePath, meta) {
    if (meta) {
        let fileMeta = context.meta[filePath];
        if (!fileMeta) {
            fileMeta = {};
            context.meta[filePath] = fileMeta;
        }
...
module.exports = {
    hooks: {
        ...
        async resolveId (context, id, parentFilePath) {
            ...
            handleMetaProperty(context, hr.id, hr.meta);
        ...
        async load (context, filePath, parentFilePath) {
            ...
            if (typeof hr === 'object' && hr.code) {
                ...
                handleMetaProperty(context, filePath, hr.meta);
        ...
        async transform (context, code, filePath) {
            ...
            let hr = await callAsyncSequentialHook(context, 'transform', (input) => {                
                ...
                handleMetaProperty(context, filePath, result.meta);
        ...
        async moduleParsed (context, moduleInfo) {
            await callAsyncParallelHook(context, 'moduleParsed', [moduleInfo]);
        }
    },
