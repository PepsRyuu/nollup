...
async function resolveId (context, target, current) {
    let result = await callPluginCompileMethod(context, PluginContext.create(context), 'resolveId', target, current);
...
async function resolveDynamicImport (context, target, current) {
    let result = await callPluginCompileMethod(context, PluginContext.create(context), 'resolveDynamicImport', target, current);
...
async function load (context, target, current) {
    let hr = await callPluginCompileMethod(context, PluginContext.create(context), 'load', target);
...
async function transform (context, original_code, filepath) {
    ...
    for (let i = 0; i < plugins.length; i++) {
        let result = await callPluginMethod(PluginContext.create(context), plugins[i], 'transform', transpiled_code, filepath);
