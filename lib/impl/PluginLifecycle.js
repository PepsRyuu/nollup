...
async function callPluginCompileMethod (context, current_filepath, method, ...args) {
    ...
    for (let i = 0; i < plugins.length; i++) {
        ...
        let thisValue = pluginsContext[i];
...
function setCurrentChunk (context, chunk) {
    context.plugins.forEach((p, i) => {
        context.pluginsContext[i].__current_chunk = chunk;
    });
}

function getCurrentChunk (context) {
    if (context.plugins.length > 0) {
        return context.pluginsContext[0].__current_chunk;
    }
...
function setCurrentFile (context, file) {
    context.plugins.forEach((p, i) => {
        context.pluginsContext[i].__current_filepath = file;
    });
}

function getCurrentFile (context) {
    if (context.plugins.length > 0) {
        return context.pluginsContext[0].__current_filepath;
    }
