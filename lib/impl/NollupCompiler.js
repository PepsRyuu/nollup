...
let { getNameFromFileName, emitAssetToBundle, formatFileName } = require('./utils');
 ...
 */
async function compileModule (context, filePath, parentFilePath, depth, emitted, bundleModuleIds, generator, syntheticNamedExports, isEntry, bundleEmittedAssets) {
    if (depth >= 255) {
    ...
    if (file.invalidate) {
        ...
        let resolved = await ImportExportResolver(context.plugins, transformed.code, filePath, generator);
    ...
    for (let i = 0; i < file.imports.length; i++) {
        try {
            await compileModule(context, file.imports[i].source, filePath, depth + 1, emitted, bundleModuleIds, generator, file.imports[i].syntheticNamedExports, false, bundleEmittedAssets);
        } catch (e) {
    ...
    if (file.invalidate) {
        file.code = generator.onGenerateModule(context.files, filePath);
...
async function compileInputTarget (context, filePath, bundleModuleIds, generator, bundleEmittedAssets, isEntry) {
    ...
    await compileModule(context, filePath, parentFilePath, depth, emitted, bundleModuleIds, generator, false, isEntry, bundleEmittedAssets);
    return emitted;
}
...
module.exports = {
    ...
    async compile (context, generator) {
        ...
        let addBundleEmittedChunks = function (dynamicImports, emittedChunks) {
        ...
        try {
            ...
            modules = Object.entries(context.files).reduce((acc, val) => {
                ...
                acc[id] = {
                    index: file.index,
                    code: generator.onGenerateModulePreChunk(file, bundle)
                };
                return acc;
            }, {});
