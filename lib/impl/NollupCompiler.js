...
async function compileInputTarget (context, filePath, bundleModuleIds, generator, bundleEmittedAssets, isEntry) {
    ...
    function hoistDependencies (deps) {
        deps.forEach(d => {
            ...
            if (!depFile.hoist) {
                if (emitted.invalidate.indexOf(depFile) === -1) {
                    emitted.invalidate.push(depFile);
                }
                depFile.hoist = true;
                hoistDependencies(depFile.imports.map(i => i.source));
            }
        });
    }
    
    emitted.circulars.forEach(trace => {
        hoistDependencies(trace);
    });
...
module.exports = {
    ...
    async compile (context) {
        ...
        try {
            ...
            for (let i = 0; i < inputs.length; i++) {
                ...
                bundle.push({
                    ...
                    dynamicImports: [],
                    exports: context.files[file].exports.map(e => e.exported),
                    facadeModuleId: file,
            ...
            for (let i = 0; i < bundleEmittedChunks.length; i++) {
                ...
                let bundleEntry = {
                    ...
                    dynamicImports: [],
                    exports: context.files[chunk.id].exports.map(e => e.exported),
                    implicitlyLoadedBefore: [], 
                    referencedFiles: [], 
                    isImplicitEntry: false
                };
        ...
        if (bundleCirculars.length > 0) {
            ...
            show = show.map(t => t.map(t => t.replace(process.cwd(), '')).join(' -> '));
