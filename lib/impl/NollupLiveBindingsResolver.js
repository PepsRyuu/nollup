...
function transformImportReferences (imports, ast, generator) {
    let ancestors = [];
    let scope_variables = [];
        ...
        for (let i = 0; i < nodes.length; i++) {
            ...
            if (node.type === 'Identifier' && imports.indexOf(node.name) > -1) {  
                ...
                if (
                    ...
                    generator.onESMImportLiveBinding(node, ancestors);
                }
 ...
 */
function NollupLiveBindingsResolver (imports, ast, generator) {
    let specifiers = imports.flatMap(i => i.specifiers.map(s => s.local));
    transformImportReferences(specifiers, ast, generator);
}
