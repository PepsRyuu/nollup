 ...
 * @return {string}
 */
function getExportFrom (s, namespace, local) {
    if (s.imported === '*') {
        return `for(var __k__ in ${namespace}){__k__ !== "default" && (__e__(__k__, ${namespace}[__k__]))}`
    } else {
        return `__e__("${local.slice(3)}", ${local})`;
    }
}
...
class NollupCodeGenerator {
    ...
    onESMNodeFound (node, args) {
        ...
        if (node.type === 'ExportDefaultDeclaration') {
            ...
            if (node.declaration && node.declaration.id) {
                ...
                activeESMOutput.appendRight(node.declaration.end, `; __e__('default', ${node.declaration.id.name});`);
            } else {
                activeESMOutput.overwrite(node.start, node.start + offset, `__e__('default', `, { contentOnly: true });
                let end = activeESMCode[node.end - 1] === ';'? node.end - 1 : node.end;
                activeESMOutput.appendRight(end, ');');
            }
        ...
        if (node.type === 'ExportNamedDeclaration') {
            ...
            if (!node.declaration && node.specifiers) {
                if (!node.source) {
                    ...
                    activeESMOutput.overwrite(node.start, node.end, blanker(activeESMCode, node.start, node.end), { contentOnly: true });
                }
            }

            let specifiers = '; ' + args.map(e => `__e__('${e.exported}', ${e.local});`).join('');
        ...
        if (node.type === 'ImportExpression') {
            if (!args.external) {
                if (typeof args.resolved.id === 'string' && path.isAbsolute(args.resolved.id)) {
                    ...
                    activeESMOutput.overwrite(node.source.start, node.source.end, '\'' + args.resolved.id.replace(/\\/g, '\\\\') + '\'', { contentOnly: true });
                }
            }
        } 
    }
     ...
     * @return {string} 
     */
    onGenerateModule (modules, filePath ) {
        let { code, map, imports, externalImports, dynamicImports, syntheticNamedExports } = modules[filePath];
        ...
        code += `//# sourceURL=nollup-int:///${sourcePath}`;

        return `
            function (__c__, __r__, __d__, __e__) {
                ${imports.map((i, index) => {
                    let id = `_i${index}`;
                    return `var ${id}; ${i.specifiers.map(s => 'var ' + getLocalSpecifier(id, i, s)).join(';')};`
                }).join('; ')}
                ...
                ${externalImports.map(i => {
                    ...
                    return i.specifiers.map(s => {
                        ...
                        if (i.export)
                            output += getExportFrom(s, id, local);
                ...
                __d__(function () {
                    ${imports.map((i, index) => {
                        ...
                        return i.specifiers.map(s => {
                            let local = getLocalSpecifier(id, i, s);
                            let output = `${local} = ${id}()${s.imported === '*'? '' : `.${s.imported}`}`;

                            if (i.export)
                                output += ';' + getExportFrom(s, local, local) + ';';
     ...
     * @return {string}
     */
    onGenerateModulePreChunk (file, bundle) {
        // Resolve dynamic imports
        if (file.dynamicImports.length > 0) {
            return file.code.replace(/require\.dynamic\(\\\'(.*?)\\\'\)/g, (match, inner) => {
                return 'require.dynamic(\\\'' + bundle.find(b => {
                    return /** @type {RollupOutputChunk} */ (b).facadeModuleId === inner
                }).fileName +'\\\')';
            });
        }
