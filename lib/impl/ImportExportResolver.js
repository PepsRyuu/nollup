...
let { isExternal, findChildNodes } = require('./utils');
...
function resolveDefaultExport (context, input, output, node) {
    ...
    if (node.declaration && node.declaration.id) {
        ...
        output.transpiled.appendRight(node.declaration.end, `; __e__('default', function () { return ${node.declaration.id.name} });`);
    } else {
        output.transpiled.overwrite(node.start, node.start + offset, `__e__('default', function () { return `, { contentOnly: true });
...
async function resolveNamedExport (context, input, output, node, currentpath) {
    ...
    if (node.declaration) {
        ...
        if (dec.id) {
            output.transpiled.appendRight(dec.end, `; __e__('${dec.id.name}', function () { return ${dec.id.name} });`);
            ...
            let transpiled = '; ' + dec.declarations.map(node => {
                if (node.id.type === 'ObjectPattern') {
                    return node.id.properties.map(prop => {
                        ...
                        return `__e__('${prop.value.name}', function () { return ${prop.value.name} })`
                    }).join(', ');
                } else {
                    output.exports.push(node.id.name);

                    if (!context.liveBindings && !node.init) {
                        output.exportLiveBindings.push(node.id.name);
                        return ``;
                    }

                    return `__e__('${node.id.name}', function () { return ${node.id.name} })`;
                }
            }).join(', ') + ';';
    ...
    if (!node.declaration && node.specifiers) {
        ...
        node.specifiers.forEach(spec => {
            if (spec.type === 'ExportSpecifier') {
                if (!node.source) {
                    transpiled += `__e__('${spec.exported.name}', function () { return ${spec.local.name} });`;
                }
...
function walkLightLiveBindings (context, input, output, nodes, found, level) {
    for (let i = 0; i < nodes.length; i++) {
        let node = nodes[i];
        let locals = [];
        ...
        walkLightLiveBindings(context, input, output, findChildNodes(node), found, level + 1);
...
module.exports = async function (context, input, currentpath) {
    ...
    if (!context.liveBindings && output.exportLiveBindings.length > 0) {
        walkLightLiveBindings(context, input, output, ast.body, [], 0);
    }
