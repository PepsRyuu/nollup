...
function resolveMetaProperty (context, input, output, node, currentpath) {
    if (node.meta_property) {
        output.transpiled.overwrite(node.start, node.property.end, '__nollup__import__meta__');
    } else {
        output.transpiled.overwrite(node.start, node.property.end, '__nollup__import__meta__.null');
    }
...
async function resolveDynamicImport (context, input, output, node, currentpath) {    
    ...
    if (typeof resolved.id === 'string' && path.isAbsolute(resolved.id)) {
        ...
        output.transpiled.overwrite(arg.start, arg.end, '\'' + resolved.id.replace(/\\/g, '\\\\') + '\'');
